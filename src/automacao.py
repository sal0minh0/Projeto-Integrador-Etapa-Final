...
class ServicoProcessamento:
    ...
    def avaliar_e_processar_tarefa(self, identificador_tarefa: str) -> AcaoRobo:
        ...
        if not tarefa_encontrada:
            logger.error("Tarefa %s não encontrada no repositório.", identificador_tarefa)
            raise ValueError(f"Tarefa {identificador_tarefa} não encontrada no repositório.")

        logger.info(
            "Processando tarefa %s (Status: %s).",
            identificador_tarefa,
            tarefa_encontrada.status.value
        )
        acao_decidida = self.controlador.determinar_acao(tarefa_encontrada)

        if acao_decidida == AcaoRobo.REPROCESSAR:
            logger.warning("Tarefa %s encaminhada para reprocessamento.", identificador_tarefa)
            tarefa_encontrada.registrar_tentativa()
            self.repositorio.salvar(tarefa_encontrada)
        elif acao_decidida == AcaoRobo.BAIXA_SISTEMA:
            logger.info("Baixa no sistema efetuada para tarefa %s.", identificador_tarefa)
        elif acao_decidida == AcaoRobo.ESCALAR_HUMANO:
            logger.error(
                "Tarefa %s escalada para humano (Tentativas: %d).",
                identificador_tarefa,
                tarefa_encontrada.tentativas
            )
