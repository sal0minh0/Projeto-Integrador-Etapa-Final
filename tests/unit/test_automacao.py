"""Módulo de testes unitários para o controlador e entidades de automação."""
import pytest
from src.automacao import (
    Tarefa, ControladorRobo, StatusTarefa, AcaoRobo,
    PlataformaDestino, TarefaInvalidaError
)

# pylint: disable=redefined-outer-name

class TestTarefa:
    """Suíte de testes para a entidade Tarefa."""
    ...
    def test_registrar_tentativa(self):
        ...
        tarefa = Tarefa(
            id_tarefa="T-004", status=StatusTarefa.ERRO, urgente=False, tentativas=1
        )
...
class TestControladorRobo:
    """Suíte de testes para a classe ControladorRobo."""
    ...
    def test_acao_baixa_sistema_para_tarefas_concluidas(self, controlador):
        ...
        tarefa = Tarefa(
            id_tarefa="T-100", status=StatusTarefa.CONCLUIDO, urgente=False, tentativas=1
        )
    ...
    def test_acao_enviar_fila_tarefas_pendentes_nao_urgentes(self, controlador):
        ...
        tarefa = Tarefa(
            id_tarefa="T-105", status=StatusTarefa.PENDENTE, urgente=False, tentativas=0
        )
        acao = controlador.determinar_acao(tarefa)
    ...
    def test_acao_enviar_fila_tarefas_travadas_em_processamento(self, controlador):
        ...
        tarefa = Tarefa(
            id_tarefa="T-106", status=StatusTarefa.EM_PROCESSAMENTO, urgente=False, tentativas=1
        )
        acao = controlador.determinar_acao(tarefa)
        assert acao == AcaoRobo.ENVIAR_FILA
    ...
    def test_acao_escalar_humano_status_invalido(self, controlador):
        ...
        tarefa = Tarefa(
            id_tarefa="T-107", status=None, urgente=False, tentativas=0 # type: ignore
        )
        acao = controlador.determinar_acao(tarefa)
        assert acao == AcaoRobo.ESCALAR_HUMANO
