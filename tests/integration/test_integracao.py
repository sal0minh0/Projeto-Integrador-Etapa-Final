"""Módulo de testes de integração para o serviço de automação."""
import pytest
from src.automacao import (
    Tarefa, ControladorRobo, StatusTarefa, AcaoRobo, PlataformaDestino,
...
@pytest.fixture
def controlador():
    """Fixture para fornecer um controlador padrão."""
    return ControladorRobo(max_tentativas=3)

@pytest.fixture
def servico(repositorio, controlador):
    """Fixture para fornecer o serviço de processamento configurado."""
    return ServicoProcessamento(repositorio, controlador)

class TestIntegracaoServicoAutomacao:
    """Suíte de testes de integração para a classe ServicoProcessamento."""
